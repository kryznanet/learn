...
window.addEventListener("DOMContentLoaded", function () {
  ...
  style.textContent = `
    ...
    .k-table-tools button, .k-cell-tools button { border:1px solid #cbd5e1; background:#fff; border-radius:6px; padding:4px 7px; font-size:11px; cursor:pointer; }
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const exec = (command, value = null) => { focusEditor(); try { document.execCommand(command, false, value); } catch(e) { console.warn("Format command gagal:", command, e); } refreshPreview(); syncToolbarState(); };
  ...
  const block = document.getElementById("block"); if(block){ block.onmousedown=e=>e.preventDefault(); block.onchange=e=>exec("formatBlock",e.target.value); }
  const font = document.getElementById("font"); if(font){ font.onmousedown=e=>e.preventDefault(); font.onchange=e=>exec("fontName",e.target.value); }
  const size = document.getElementById("size"); if(size){ size.onmousedown=e=>e.preventDefault(); size.onchange=e=>{ focusEditor(); try{document.execCommand("styleWithCSS",false,true);}catch(_){} try{document.execCommand("fontSize",false,"7"); editor.querySelectorAll('font[size="7"]').forEach(node=>{const span=document.createElement("span"); span.style.fontSize=e.target.value; span.innerHTML=node.innerHTML; node.replaceWith(span);});}catch(_){applyInlineStyle("fontSize",e.target.value);} refreshPreview(); syncToolbarState();}; }
  const color = document.getElementById("color"); if(color){ color.onmousedown=e=>e.preventDefault(); color.oninput=e=>exec("foreColor",e.target.value); }
  const hilite = document.getElementById("hilite"); if(hilite){ hilite.onmousedown=e=>e.preventDefault(); hilite.oninput=e=>{focusEditor(); try{document.execCommand("hiliteColor",false,e.target.value);}catch(_){document.execCommand("backColor",false,e.target.value);} refreshPreview();}; }
  ...
  const image=document.getElementById("image"); if(image){ image.onmousedown=e=>e.preventDefault(); image.onclick=()=>{}; }
  ...
  let selectedImage=null; editor.addEventListener("click",e=>{if(selectedImage)selectedImage.classList.remove("k-selected-image");selectedImage=e.target.closest("img");if(selectedImage&&editor.contains(selectedImage))selectedImage.classList.add("k-selected-image");else selectedImage=null;});
...
window.addEventListener("DOMContentLoaded", function () {
  const editor=document.getElementById("konten"); const toolbar=editor?.closest(".editor")?.querySelector(".toolbar");
  if(!editor||!toolbar||editor.dataset.kAdvancedTable==="1")return; editor.dataset.kAdvancedTable="1";
...
window.addEventListener("DOMContentLoaded", function () {
  const editor=document.getElementById("konten"); const toolbar=editor?.closest(".editor")?.querySelector(".toolbar");
  if(!editor||!toolbar||editor.dataset.kEditorPro==="1")return; editor.dataset.kEditorPro="1";
  const refresh=()=>{if(typeof window.preview==="function")window.preview();editor.dispatchEvent(new Event("input",{bubbles:true}));};
  ...
  const imageFromSelection=()=>{const n=window.getSelection()?.anchorNode;const el=n?.nodeType===1?n:n?.parentElement;return el?.closest?.("img");};
  ...
  const updateCellTools=()=>cellTools.classList.toggle("visible",!!cellFromSelection()); editor.addEventListener("click",updateCellTools);editor.addEventListener("keyup",updateCellTools);document.addEventListener("selectionchange",updateCellTools);
  ...
  let handle=null,selected=null,startX=0,startWidth=0;
  ...
  document.addEventListener("mousedown",e=>{if(!handle||e.target!==handle||!selected)return;e.preventDefault();e.stopPropagation();startX=e.clientX;startWidth=selected.getBoundingClientRect().width;const move=ev=>{const width=Math.max(40,Math.min(2400,Math.round(startWidth+ev.clientX-startX)));selected.style.width=width+"px";selected.style.height="auto";const r=selected.getBoundingClientRect();handle.style.left=(window.scrollX+r.right-6)+"px";handle.style.top=(window.scrollY+r.bottom-6)+"px";};const up=()=>{document.removeEventListener("mousemove",move);document.removeEventListener("mouseup",up);refresh();};document.addEventListener("mousemove",move);document.addEventListener("mouseup",up);});
  editor.addEventListener("keydown",e=>{if(e.key==="Escape")removeHandle();});
});

function roleLabel(role){return({super_admin:"Super Admin",admin:"Admin",penulis:"Penulis / Pemateri",viewer:"Viewer"}[role]||role);}

window.addEventListener("DOMContentLoaded", async function () {
  try {
    const {data:{user}}=await supabaseClient.auth.getUser(); if(!user)return;
    const {data:staff,error}=await supabaseClient.from("admin_users").select("role,active,display_name").eq("user_id",user.id).maybeSingle(); if(error||!staff)return;
    ...
    const role=String(staff.role||"viewer");document.body.classList.add("k-role-"+role);const top=document.querySelector(".top");
    ...
    if(role==="super_admin"&&nav&&content&&!document.querySelector('[data-view="users"]')){const btn=document.createElement("button");btn.type="button";btn.dataset.view="users";btn.textContent="👥 Kelola User";btn.addEventListener("click",()=>{if(typeof window.showView==="function")window.showView("users");});nav.appendChild(btn);const section=document.createElement("section");section.id="users";section.className="view";section.innerHTML='<div id="k-users-shell" class="card"><iframe id="k-users-frame" title="Kelola User" src="users.html"></iframe></div>';content.appendChild(section);}
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const input=document.createElement("input"); input.type="file"; input.accept="image/jpeg,image/png,image/webp,image/gif"; input.hidden=true; input.id="k-image-upload-input"; document.body.appendChild(input);
  let savedRange=null;
  const saveSelection=()=>{const selection=window.getSelection();if(!selection||!selection.rangeCount)return;const range=selection.getRangeAt(0);if(editor.contains(range.commonAncestorContainer))savedRange=range.cloneRange();};
  const restoreSelection=()=>{const selection=window.getSelection();selection.removeAllRanges();if(savedRange)selection.addRange(savedRange);editor.focus();};
  imageButton.addEventListener("mousedown",e=>{e.preventDefault();saveSelection();}); imageButton.addEventListener("click",e=>{e.preventDefault();e.stopImmediatePropagation();saveSelection();input.click();},true);
  input.addEventListener("change",async()=>{
    const file=input.files?.[0];input.value="";if(!file)return;
    if(!/^image\/(jpeg|png|webp|gif)$/i.test(file.type)){alert("Format gambar harus JPG, PNG, WebP, atau GIF.");return;}
    if(file.size>25*1024*1024){alert("Ukuran gambar maksimal 25 MB.");return;}
    const original=imageButton.textContent;imageButton.disabled=true;imageButton.textContent="⏳ Upload...";
    try{
      const {data:{user}}=await supabaseClient.auth.getUser();if(!user)throw new Error("Sesi login tidak ditemukan. Silakan login ulang.");
      const safeName=file.name.replace(/[^a-zA-Z0-9._-]+/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")||"gambar";
      const path=`images/${user.id}/${Date.now()}-${safeName}`;
      const {error}=await supabaseClient.storage.from("materi-files").upload(path,file,{cacheControl:"3600",contentType:file.type,upsert:false});if(error)throw error;
      const {data}=supabaseClient.storage.from("materi-files").getPublicUrl(path);const publicUrl=data?.publicUrl;if(!publicUrl)throw new Error("URL gambar tidak berhasil dibuat.");
      restoreSelection();document.execCommand("insertImage",false,publicUrl);editor.dispatchEvent(new Event("input",{bubbles:true}));if(typeof window.preview==="function")window.preview();imageButton.textContent="🖼️ Upload";
    }catch(error){console.error("Upload gambar gagal:",error);alert("Upload gambar gagal: "+(error?.message||"Kesalahan tidak diketahui"));imageButton.textContent=original||"🖼️ Upload";}
