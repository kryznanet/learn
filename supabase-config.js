...
(function () {
  ...
  if (!window.__kryznaLoadingHooks) {
    ...
    function ensureUi() {
      ...
      if (!document.getElementById('kryzna-loading-style')) {
        ...
        style.textContent = `
          ...
          body.k-loading-active {
            cursor: wait;
          }

          body.k-loading-active > *:not(#kryzna-loading) {
            pointer-events: none;
          }
        `;
      ...
      overlay.innerHTML = `
        <div class="k-loading-card">
          <div class="k-spinner"></div>
          <div id="k-loading-text">Memproses...</div>
        </div>
      `;
    ...
    function show(label) {
      ...
      if (text) text.textContent = label || 'Memproses...';
    ...
    function hide() {
      ...
      timer = setTimeout(() => {
        ...
        if (document.body) document.body.classList.remove('k-loading-active');
        started = 0;
      }, wait);
    }
    ...
    window.fetch = function (...args) {
      ...
      if (/\/storage\/v1\//i.test(requestUrl)) {
        label = 'Mengunggah / memproses file...';
      } else if (/\/rest\/v1\//i.test(requestUrl)) {
        label = 'Memproses data...';
      } else if (/\/auth\/v1\//i.test(requestUrl)) {
